...
const state = {
  ...
  itemsDisabledCount: 0,
  movementRollFreezeCount: 0,
  tokenPickupFrozenCount: 0,
  bulletsCombatFrozenCount: 0,
  pillowFightCount: 0,
  // doubleMovementCount: multi-turn doubling (set to players.length+1, decrements each turn-end).
  // doubleMovementThisTurn: one-shot flag from a single-use card; cleared immediately after the roll doubles.
  // Both can be active simultaneously — the roll is doubled if EITHER is non-zero/true.
  doubleMovementCount: 0,
  doubleMovementThisTurn: false,
  blockedSewerSpaces: new Map(),
  lastPlayedWeaponName: null,
  lastPlayedWeaponByPlayerId: null,
  lastPlayedEventCard: null,
  // movementBonus: flat value added to the roll (cleared when the roll is consumed).
  // moveFloorThisTurn: minimum the roll can produce — move = Math.max(roll+bonus, floor).
  // Both are cleared together at roll time; neither persists past the ROLL_MOVE step.
  movementBonus: 0,
  moveFloorThisTurn: 0,
  ...
  pendingTileDeck: TILE_DECK.BASE, // TILE_DECK.BASE | collKey — which deck the current pending tile came from
...
// Build pendingEventChoice `options` array for player selection.
// Provide an optional labelFn(player) to customise each label; defaults to player.name.
function playerOpts(players, labelFn) {
  return players.map((p) => ({ key: `p_${p.id}`, label: labelFn ? labelFn(p) : p.name }));
}

// Resolve a player from a key generated by playerOpts (i.e. "p_<id>").
function playerFromOpt(key) {
  return state.players.find((p) => p.id === Number(key.slice(2)));
}

// Returns the id of the player who currently needs to act.
// Normally the turn player, but defers to pendingEventChoice / pendingZombieDiceChallenge
// when those require a non-current player to respond.
function getActivePlayerId() {
  if (state.pendingEventChoice) return state.pendingEventChoice.playerId;
  if (state.pendingZombieDiceChallenge) return state.pendingZombieDiceChallenge.targetPlayerId;
